...
{    
    ...
    let scene = this;
// Carga y crea todas las píldoras del mapa como objetos interactivos para que el jugador las pueda consumir.
...
function update()
{
    ...
    if(player.active) {
        // Si el jugador se mueve más allá del borde izquierdo, su posición se coloca al final de la pantalla en el lado derecho.
        if(player.sprite.x < 0 - offset ) {            
            player.sprite.setPosition(width + offset, player.sprite.y);
        }
        // Si el jugador se mueve más allá del borde derecho, su posición se coloca al inicio de la pantalla en el lado izquierdo.
...
function getDirection(map, layer, sprite) {
    ...
    let currentTile = map.getTileAtWorldXY(sx, sy, true);  //encontramos la celda exacta del mapa en la que se encuentra el sprite.
    if(currentTile) {
